...
### Phase 1 ― 翼型のブレンドと粘性極曲線（ポーラ）の取得

#### 1-1. 翼型座標の線形補間（ブレンディング）
...
### Phase 2 ― 最小誘導損失理論（MIL）による最適プロペラ形状の設計
...
#### 2-3. ラーラビーの最小誘導損失理論（Larrabee MIL）
...
### Phase 4 ― 構造特性（断面積・断面二次モーメント）の解析
...
graph TD
    ...
    subgraph Phase2 [Phase 2: XROTOR 形状最適化]
        ...
        G -- MIL最適化計算 --> H[弦長分布 c/R, ピッチ角分布 beta の決定]
        H --> I[設計結果テキストの出力]
    end
